...
#--------------------------NOTIFIED_UPDATE_GROUP---------------------
		...
		if op.param2 in Bots:
		    pass
                    ...
                    cl.updateGroup(X)
                    Ti = cl.reissueGroupTicket(op.param1)
                    cl.acceptGroupInvitationByTicket(op.param1,Ti) #kicker join
                    X.preventJoinByTicket = True
                    cl.updateGroup(X)
                    cl.kickoutFromGroup(op.param1,[op.param2])
...
#--------------------------------------------------------
		...
		cl.sendText(msg.to,"Itu Yang Bikin BOT")
                ...
                cl.sendMessage(msg)
		cl.sendText(msg.to,"Itu Admin BOT")
#--------------------------------------------------------
...
#--------------------------------------------------------
            ...
            elif msg.text in ["Like start"]:
              if wait["likeOn"] == True:
                print "[Command]Like executed"
                cl.sendText(msg.to,"Like All Status")
                try:
                  autolike()
                except:
                  pass
...
#--------------------------------------------------------
	    ...
	    elif "Budak on" in msg.text:
		     wait["Budak"] = True
		     cl.sendText(msg.to,"Budak Sudah Aktif")

	    elif "Budak off" in msg.text:
		     wait["Budak"] = False
		     cl.sendText(msg.to,"Budak Sudah Di Nonaktifkan")
#---------------------------------------------------------
...
#--------------------------------------------------------
                ...
                if wait["Sambutan"] == True:
                    if wait["lang"] == "JP":
                        cl.sendText(msg.to,"Welcome Di Aktifkanヾ(*´∀｀*)ﾉ")
                else:
                    wait["Sambutan"] = True
                    if wait["lang"] == "JP":
                ...
                if wait["Sambutan"] == False:
                    if wait["lang"] == "JP":
                        cl.sendText(msg.to,"Welcome Di Nonaktifkan(　＾∇＾)")
                else:
                    wait["Sambutan"] = False
                    if wait["lang"] == "JP":
...
#--------------------------------------------------------
		...
		if wait["Budak"] == True: md+="✔️ Budak : On\n"
		else:md+="❌ Budak : Off\n"
		if wait["likeOn"] == True: md+="✔️ Auto Like : On\n"
...
#--------------------------------------------------------
		...
		if msg.from_ in admin:
                    ...
                    cl.acceptGroupInvitationByTicket(msg.to,Ti) #kicker join
                    G = cl.getGroup(msg.to)
                    G.preventJoinByTicket = True
                    cl.updateGroup(G)
                ...
                if targets ==[]:
                        ...
                        for target in targets:
                                try:
                                        ...
                                        cl.sendText(msg.t,"Ter ELIMINASI....")
                                        cl.sendText(msg.to,"WOLES brooo....!!!")
                                        cl.leaveGroup(msg.to)
                                        gs = cl.getGroup(msg.to)
